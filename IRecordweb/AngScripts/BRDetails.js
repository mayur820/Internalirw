...
app.controller("myctrn", function ($scope, $http, $localStorage, $sessionStorage, $window) {
    ...
    $scope.fn_Cal_GrossAmt = function () 
    {
        try 
        {
            ...
            for (i = 0; i < $scope.details.length; i++) 
            {
                Qty = 0;
                GRate = 0;
                Qty = parseFloat($scope.details[i].Qty);
                GRate = parseFloat($scope.details[i].GrossRate);
    ...
    $scope.fn_Cal_BrokAmt = function () 
    {
        ...
        try 
        {
            ...
            {
                Qty = parseFloat($scope.details[i].Qty);
                BUnit = parseFloat($scope.details[i].brkpunit);
                $scope.details[i].BrokAmt = parseFloat(Qty * BUnit).toFixed(2);
                // Gross = parseFloat($scope.TotalGrossAmount) - parseFloat( $scope.details[i].BrokAmt);
            };
            // $scope.TotalGrossAmount = parseFloat(Gross).toFixed(2);
        } 
        catch{} 
    }
    ...
    $scope.fn_Cal_NetAmount = function () {
        try {
            ...
            for (i = 0; i < $scope.details.length; i++) 
            {
                if ($scope.details[i].Type == "Sold") 
                {
                    NetAmt = $scope.details[i].GrossAmt - $scope.details[i].BrokAmt;
                    $scope.details[i].Amount  =  parseFloat(NetAmt).toFixed(2);
                }
                else if($scope.details[i].Type == "Bought")
                {
                    NetAmt = $scope.details[i].GrossAmt + $scope.details[i].BrokAmt;
                    $scope.details[i].Amount  =  parseFloat(NetAmt).toFixed(2);
                }
    ...
    debugger;
        ...
        for (i = 0; i < $scope.Exps.length; i++) {
            total = total + parseFloat($scope.Exps[i].ExpAmount);
        };
        $scope.TotalExpenses = parseFloat(total).toFixed(2);
       
     ////   if($scope.Type=="Bought")
     ////   {
     //  // netcal = parseFloat($scope.TotalOfNetAmount) + parseFloat($scope.TotalExpenses);
     //  //netcal = parseFloat($scope.TotalGrossAmount) + parseFloat($scope.TotalExpenses);
     //   for (i = 0; i < $scope.details.length; i++) {
     ...
     //   $scope.TotalOfNetAmount = parseFloat(netcal).toFixed(2); // Commented by Poonam For Net Amount 
       
    }
    $scope.Calculate_Net_Amount = function () {
    debugger;
        var total = 0, netcal= 0;
        for (i = 0; i < $scope.details.length; i++) {
            total = total + parseFloat($scope.details[i].Amount);
        }
           netcal = parseFloat($scope.TotalGrossAmount) - parseFloat($scope.TotalExpenses);   
           //alert(netcal);
    ...
    $scope.Calculate_GrossAmt = function () 
    {
        ...
        for (i = 0; i < $scope.details.length; i++) 
        {
           ...
           if (Bought > Sold) 
           {
            ...
            //  $scope.TotalOfNetAmount = parseFloat($scope.TotalGrossAmount) - parseFloat($scope.TotalExpenses);  
           }
          else 
          {
    ...
    var ex =  0;
        ...
        var FinalIT = 0, FinalIT1=0;
        var total_TotalInTrade1 = 0;
        var total_TotalInInvestment1 = 0;
        var Sold_total_TotalInInvestment=0;
        var Buy_total_TotalInInvestment=0;
        for (i = 0; i < $scope.details.length; i++) 
        {
               ...
               // //total_TotalInInvestment = total_TotalInInvestment + parseFloat($scope.details[i].Amount).toFixed(2);
               // //total_TotalInInvestment = +parseFloat(total_TotalInInvestment).toFixed(2) + + parseFloat($scope.details[i].Amount).toFixed(2);
               // total_TotalInInvestment = + total_TotalInInvestment + + parseFloat($scope.details[i].Amount).toFixed(2);
               // //alert(total_TotalInInvestment);
               //}
               //else if($scope.details[i].HoldingType == "1")
               //{
               // total_TotalInTrade = + total_TotalInTrade +  + parseFloat($scope.details[i].Amount).toFixed(2);
               //}
         
     
            if ($scope.details[i].Type == "Sold") 
            {
                Sold_total_TotalInInvestment = $scope.details[i].GrossAmt - $scope.details[i].BrokAmt;
                //total_TotalInInvestment =total_TotalInInvestment + parseFloat($scope.details[i].Amount).toFixed(2);
                $scope.details[i].Amount =  parseFloat(Sold_total_TotalInInvestment).toFixed(2);
                if($scope.details[i].HoldingType == "0")
                {
                 //total_TotalInInvestment = total_TotalInInvestment + parseFloat($scope.details[i].Amount).toFixed(2);
                 total_TotalInInvestment = +parseFloat(total_TotalInInvestment).toFixed(2) + + parseFloat($scope.details[i].Amount).toFixed(2);
                ...
                {
                 total_TotalInTrade = + parseFloat(total_TotalInTrade).toFixed(2) + + parseFloat($scope.details[i].Amount).toFixed(2);
                }
            }
            else if($scope.details[i].Type == "Bought")
            { 
                 Buy_total_TotalInInvestment = +parseFloat($scope.details[i].GrossAmt) + +parseFloat($scope.details[i].BrokAmt);
                 $scope.details[i].Amount = parseFloat(Buy_total_TotalInInvestment).toFixed(2);
                if($scope.details[i].HoldingType == "0")
                {
                 total_TotalInInvestment1 = + parseFloat(total_TotalInInvestment1).toFixed(2) + + parseFloat($scope.details[i].Amount).toFixed(2);
                }
                else if($scope.details[i].HoldingType == "1")
                {
                 total_TotalInTrade1 = + total_TotalInTrade1 + + parseFloat($scope.details[i].Amount).toFixed(2);
                }
            }
          
        }

                   if (total_TotalInInvestment > total_TotalInInvestment)
                    {
                      FinalIT= total_TotalInInvestment - total_TotalInInvestment1;
                    }
                    else
                    {
                       FinalIT= total_TotalInInvestment1 - total_TotalInInvestment;
                    }

                    if (total_TotalInTrade > total_TotalInTrade1)
                    {
                      FinalIT1= total_TotalInTrade - total_TotalInTrade1;
                    }
                    else
                    {
                       FinalIT1= total_TotalInTrade - total_TotalInTrade1;
                    }
        $scope.TotalInInvestment =  Math.abs(parseFloat(FinalIT).toFixed(2));
        $scope.TotalInTrade =  Math.abs(parseFloat(FinalIT1).toFixed(2));
